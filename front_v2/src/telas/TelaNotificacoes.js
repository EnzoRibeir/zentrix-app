 ...
 * @param {object} CORES - Cores do tema carregado
 * @returns {Array} Lista de notificações formatadas
 */
const gerarNotificacoes = (transacoes, CORES) => {
...
const TelaNotificacoes = ({ navigation }) => {
  ...
  const notificacoes = useMemo(() => {
    const todas = gerarNotificacoes(transacoes, CORES);
    if (filtroAtivo === 'Todas') return todas;
    return todas.filter((n) => n.tipo === filtroAtivo);
  }, [transacoes, filtroAtivo, CORES]);
