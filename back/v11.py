    ...
    try:
        with conn.cursor() as cursor:
            ...
            # --- CASO 2: ROTA DE AUTENTICAÇÃO WEB ---
                ...
                cursor.execute("INSERT INTO sessions (token, user_id, expires_at) VALUES (%s, %s, %s)", (session_token, str(telegram_id), expires_at))
                conn.commit()
            ...
            # --- CASO 3: IMPORTAÇÃO DE CSV (C6 BANK) ---
                ...
                if not validar_sessao(event, user_id):
                    return {'statusCode': 401, 'headers': {'Access-Control-Allow-Origin': '*'}, 'body': json.dumps({'erro': 'Nao autorizado.'})}
...
# --- ENDPOINT: Página HTML de Login do Telegram ---
...
def tratar_telegram_callback(params):
    ...
    e redireciona para o deep link do app dinamicamente.
    """
    app_redirect_uri = params.get('app_redirect_uri', 'zentrix://auth/callback')
    ...
    def redirect_error(error_code):
        ...
        return {
            'statusCode': 302,
            'headers': {'Location': f'{app_redirect_uri}{separator}error={error_code}'},
            'body': ''
        }
    ...
    try:
        with conn.cursor() as cursor:
            ...
            separator = '&' if '?' in app_redirect_uri else '?'
            redirect_url = app_redirect_uri + separator + callback_params
