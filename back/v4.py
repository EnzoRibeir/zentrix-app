...
def tratar_get(event):
    ...
    action = params.get('action') # Captura o novo parâmetro de ação
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # SUB-ROTA: Resumo Consolidado com IA
            if action == 'summary':
                # 1. Pega os limites atuais do usuário
                ...
                # 2. Agrupa os gastos do mês corrente por categoria (perfeito para gráficos de pizza/barras)
                sql_categorias = """
                ...
                # 3. Calcula o total bruto gasto no mês vigente
                sql_total = """
                ...
                # Formata a lista de categorias para texto legível para a IA entender
                cat_texto = ", ".join([f"{item['category']}: R$ {float(item['total'])}" for item in gastos_por_categoria])
                
                # 4. Aciona o Gemini para cruzar as métricas de banco e formular uma análise de saúde financeira
                prompt_insight = f"""
            ...
            # COMPORTAMENTO PADRÃO: Listagem de histórico completo
            else:
